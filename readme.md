...
## XYZタイルのURL
URLは、`http(s)://<ドメイン名>/xyz/<パスを任意に設定できる>/{z}/{x}/{y}.png`
- タイル座標は左上原点
- タイルURLのパスは`<TILE_SOURCE_FOLDER>`のサブフォルダとGeoTIFFのファイル名で決まる
- apikeyが設定されている場合、リクエストパラメータに`APIKey`を指定する
    - `http(s)://<ドメイン名>/xyz/<sub path>/{z}/{x}/{y}.png?APIKey=<APIキー>`
...
## WMSのURL
URLは、`http(s)://<ドメイン名>/wms/aaa/bbb/sample?SERVICE=wms&REQUEST=GetCapabilities&VERSION=1.1.1`

- タイルURLとWMSのURLは１対１で対応している
- apikeyが設定されている場合、リクエストパラメータに`APIKey`を指定する
    - `http(s)://<ドメイン名>/wms/aaa/bbb/sample?SERVICE=wms&REQUEST=GetMap&VERSION=1.1.1&APIKey=<APIキー>&.....`
