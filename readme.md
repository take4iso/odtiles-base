...
curl -X POST http(s)://<ドメイン名>/setCapabilities/aaa/bbb/ \
    -H "token:<UPLOAD_API_TOKEN>" \
    -d '{"layers": [{"file": "sample.tif", "name": "サンプルレイヤ", "legendUrl": "https://example.jp/my/legend/image.png"}]}'
```
このときのGetCapabilitiesのURLは以下  
```
http(s)://<ドメイン名>/wms/aaa/bbb/?SERVICE=WMS&REQUEST=GetCapabilities&VERSION=1.1.1
```
